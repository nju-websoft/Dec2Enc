...
class Dec2Enc(nn.Module):
    def __init__(self, model_path, vanilla):
        ...
        if 'bert' in model_path.lower() or 'xlm' in model_path.lower() or 'ernie' in model_path.lower():
        ...
        self. linear = nn.Linear(self.model.config.hidden_size, 6)
    ...
    def forward(self, input_ids,  attention_mask, labels=None):
        ...
        if labels is not None:
            ...
            for logits_i in logits:
                span_pred = []
                for i in range(len(logits_i[0]) // 2):
                    span_pred_i = []
                    start_flag = None
                    for idx, logits_i_item in enumerate(logits_i):
                        start, end = logits_i_item[i * 2: i * 2 + 2]
                        if start > 0:
                            start_flag = idx
...
def get_input_feature(features, max_source_length):
    ...
    for sample in features:
        ...
        for idx, answers_idx_i in enumerate(answers_idx):
            answers_idx_i = sorted(answers_idx_i, key=lambda x: x[0])
            beg_idxs, end_idxs = [], []
            for beg, end in answers_idx_i:
                beg_idxs.append(beg)
                end_idxs.append(end)

            while len(label_char_idxs) < (idx + 1) * 2:
                label_char_idxs.append([])
            assert len(beg_idxs) == len(end_idxs)
            label_char_idxs[idx * 2].append(beg_idxs)
            label_char_idxs[idx * 2 + 1].append(end_idxs)
    ...
    labels = np.zeros([bs, seq_len, len(label_char_idxs)])
    ...
    for i in range(len(label_char_idxs)):
        label_tokens(offset_mapping, label_char_idxs[i], i)
...
def subwordid_to_text(batch_example, spans_predict, token_idx_maps, results, golds_answers):
    for sample, spans_p, token_idx_map in zip(batch_example, spans_predict, token_idx_maps):
        ...
        for i, span_p in enumerate(spans_p):
            ...
            ans = sample['answers'][i]
            results[f'{id}-{i}'] = answers_item
            golds_answers[f'{id}-{i}'] = ans
...
def read_cmqa(path):
    ...
    for sample in dataset:
        ...
        offset = len(question)
        for answers_i in answers:
            answers_idx_char_i = []
            for answer, [beg, end] in answers_i:
                beg += offset
                end += offset
                assert context[beg: end] == answer
                answers_idx_char_i.append([beg, end])
            answers_i = [item[0] for item in answers_i]
            answers_new.append(answers_i)
            answers_idx_char.append(answers_idx_char_i)
...
if __name__ == '__main__':
    ...
    train_examples = read_cmqa(data_path_train)
    dev_examples = read_cmqa(data_path_dev)
    test_examples = read_cmqa(data_path_test)
    if debug:
    ...
    parameters = filter(lambda p: p.requires_grad, model.model.parameters())
    ...
    print('# parameters:', sum(param.numel() for param in model.parameters()) // 1000000)
